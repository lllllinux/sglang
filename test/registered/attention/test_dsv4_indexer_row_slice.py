...
from sglang.srt.layers.attention.dsv4.indexer import (
    ...
    _cap_mqa_logits_chunk_rows_for_metadata,
...
class TestDsv4IndexerRowSlice(unittest.TestCase):
    ...
    def test_sm120_metadata_row_cap_composes_with_memory_budget(self):
        metadata_chunks = [mock.sentinel.first, mock.sentinel.second]
        self.assertEqual(
            _cap_mqa_logits_chunk_rows_for_metadata(None, metadata_chunks),
            _SM120_INDEXER_M_CHUNK,
        )
        self.assertEqual(
            _cap_mqa_logits_chunk_rows_for_metadata(1024, metadata_chunks), 1024
        )
        self.assertEqual(
            _cap_mqa_logits_chunk_rows_for_metadata(8192, metadata_chunks),
            _SM120_INDEXER_M_CHUNK,
        )
        self.assertIsNone(_cap_mqa_logits_chunk_rows_for_metadata(None, None))
