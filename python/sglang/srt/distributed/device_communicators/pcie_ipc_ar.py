...
class PcieIpcCommunicator:
    ...
    def _ensure_workspace(self, inp: torch.Tensor) -> bool:
        ...
        if envs.SGLANG_PCIE_IPC_AUTOTUNE.get():
            try:
                hidden = inp.shape[-1]
                logger.info(
                    "Resolving FlashInfer PCIe-IPC all-reduce tuning "
                    "(world=%d, max_numel=%d, hidden=%d)",
                    self._world_size,
                    max_numel,
                    hidden,
                )
                configs = self._workspace.tune([hidden], dtype=torch.bfloat16)
                logger.info(
                    "FlashInfer PCIe-IPC tuning resolved (%d shapes)",
                    len(configs),
                )
            except Exception as e:
                logger.warning(
                    "FlashInfer PCIe-IPC autotune failed: %s; reductions stay "
                    "on NCCL",
                    e,
                )
