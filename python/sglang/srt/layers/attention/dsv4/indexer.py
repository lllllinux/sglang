...
def _cap_mqa_logits_chunk_rows_for_metadata(
    chunk_rows: Optional[int], deep_gemm_metadata: Any
) -> Optional[int]:
    """Honor the SM120 DeepGEMM metadata kernel's maximum row count."""
    if not isinstance(deep_gemm_metadata, list):
        return chunk_rows
    return min(chunk_rows or _SM120_INDEXER_M_CHUNK, _SM120_INDEXER_M_CHUNK)
...
class C4IndexerBackendMixin:
    ...
    def forward_c4_indexer(
        ...
        if nonpaged_plan is not None:
            ...
            chunk_rows = _cap_mqa_logits_chunk_rows_for_metadata(
                chunk_rows, indexer_metadata.deep_gemm_metadata
            )
        ...
        if logits is not None:
            ...
            for start in range(0, query_rows, chunk_rows):
                ...
                if use_fp4_indexer:
                    ...
                    q_part = q[start:end]
                if (
                    isinstance(deep_gemm_metadata, list)
                    and chunk_rows == _SM120_INDEXER_M_CHUNK
                ):
                    metadata_part = deep_gemm_metadata[
                        start // _SM120_INDEXER_M_CHUNK
                    ]
                else:
                    metadata_part = _paged_mqa_logits_metadata_slice(
